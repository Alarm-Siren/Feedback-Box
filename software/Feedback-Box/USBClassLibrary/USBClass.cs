...
//---------------------------------------------------------------------
/*
 * The code in this directory originates from "A USB Library to Detect USB
 * Devices", posted on Code Project by user slelong with a last update of 21
 * Sep 2014, link:
 * https://www.codeproject.com/Articles/60579/A-USB-Library-to-Detect-USB-Devices.
 *
 * According to that article, this code is published under "The Code Project
 * Open License (CPOL) 1.02", a copy of which is included here under CPOL.htm.
 * 
 * The code has been adapted from the original article in order to better meet
 * the requirements of this specific project and to generally tidy up the
 * codebase. As this version of the library is not directly compatible with
 * the original version, the major version number has been incremented in line
 * with semantic version numbering.
 * 
 * These modifications were undertaken by Nicholas Parks Young in 2020.
 */
using System;
...
namespace USBClassLibrary
{
    ...
    public class USBClass
    {
        ...
        private class Win32Wrapper
        {
            ...
            /// <summary>
            /// The SetupDiGetDeviceInstanceIdW function retrieves the device instance ID that is associated with a device information element.
            /// </summary>
            ...
            /// <param Name="DeviceInstanceId">A pointer to the character buffer that will receive the NULL-terminated device instance ID for the specified device information element.</param>
        ...
        /// <summary>
        /// Enumerate all USB devices and look for the device whose DevEnum, VID and PID are provided.
        /// </summary>
        /// <param name="DevEnum">The DevEnum type of device to look for (e.g. "USB" or "FTDIBUS")</param>
        ...
        /// <param name="GetComPort">If set to true, this call will also attempt to fetch the COM port associated with the device.</param>
        ...
        public static bool GetUSBDevice(String DevEnum, UInt32 VID, UInt32 PID, ref List<DeviceProperties> ListOfDP, bool GetComPort = false, Nullable<UInt32> MI = null)
        {
            IntPtr IntPtrBuffer = Marshal.AllocHGlobal(BUFFER_SIZE);
                                    ...
                                    if (RequiredSize <= BUFFER_SIZE)
                                    {
                                        if (Win32Wrapper.SetupDiGetDeviceRegistryProperty(h, ref DevInfoData, (UInt32)Win32Wrapper.SPDRP.SPDRP_HARDWAREID, ref RegType, IntPtrBuffer, BUFFER_SIZE, ref RequiredSize))
                                        {
                                                ...
                                                )
                                            {
                                                ...
                                                /*DP.DeviceMI = String.Empty;
                                                if (Win32Wrapper.SetupDiGetDeviceRegistryProperty(h, ref DevInfoData, (UInt32)Win32Wrapper.SPDRP.SPDRP_HARDWAREID, ref RegType, IntPtrBuffer, BUFFER_SIZE, ref RequiredSize))
                                                {
                                                    DP.DeviceMI = Marshal.PtrToStringAuto(IntPtrBuffer);
                                                }*/

                                                DP.DeviceInstanceID = String.Empty;
                                                if (Win32Wrapper.SetupDiGetDeviceInstanceId(h, ref DevInfoData, IntPtrBuffer, BUFFER_SIZE, ref RequiredSize))
                                                {
                                                    DP.DeviceInstanceID = Marshal.PtrToStringAuto(IntPtrBuffer);
                                                }
                                                ...
                                                if (GetComPort)
                                                {
                                                    ...
                                                    else
                                                    {
                                                        ...
                                                        {
                                                            DP.SerialComPort = Data.ToString();
                                                        }
